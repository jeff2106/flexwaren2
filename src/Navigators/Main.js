...
import HistoriqueCourses from '../Containers/Views/HistoriqueCourses'
...
const MainNavigator = () => {
  ...
  React.useEffect(() => {
    PushNotification.createChannel(
      {
        channelId:
          u_dataV?.u_data?.accountType == 'Passagers'
            ? 'Passagers'
            : 'Conducteur', // (required)
        channelName:
          u_dataV?.u_data?.accountType == 'Passagers'
  ...
  function sendNotif(data) {
    PushNotification.localNotification({
      ...
      channelId:
        u_dataV?.u_data?.accountType == 'Passagers'
          ? 'Passagers'
          : 'Conducteur', // (required) channelId, if the channel doesn't exist, notification will not trigger.
  ...
  requestMultiple([PERMISSIONS.IOS.CAMERA, PERMISSIONS.IOS.FACE_ID])
    .then(statuses => {
      if (statuses[PERMISSIONS.IOS.CAMERA] == 'blocked') {
        Alert.alert(
          'Messages ',
          "Vous devez autoriser l'accès à la caméra car vous devez configurer votre photo se profile ou prendre en photo votre permis, vous pouvez refuser de l'activer mais vous ne pourrez pas utiliser les services qui requie l'appareil photo",
          [
            {
              text: 'Activer',
              onPress: () => {
    ...
    <AuthContext.Provider value={authContext} ref={navigationRef}>
      {loginState?.userToken === null ? (
        ...
        >
          ...
          <Drawer.Screen
            name="PCodesTravailleurs"
            component={PCodesTravailleurs}
          />
          <Drawer.Screen
            name="PCodesCovoiturages"
            component={PCodesCovoiturages}
          />
          ...
          <Drawer.Screen
            name="AccueilCovoiturage"
            component={AccueilCovoiturage}
          />
          <Drawer.Screen
            name="FetchDriversCovoiturage"
            component={FetchDriversCovoiturage}
          />
          <Drawer.Screen name="CourseStart" component={CourseStart} />

          <Drawer.Screen
            name="AccueilCovoiturageP"
            component={AccueilCovoiturageP}
          />
          <Drawer.Screen name="RaceCourseStart" component={RaceCourseStart} />
          <Drawer.Screen
            name="RaceStartFindCustomers"
            component={RaceStartFindCustomers}
          />
