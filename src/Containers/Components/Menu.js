    ...
    <View style={styles.Container}>
      ...
      <Text
        style={[
          styles.bold,
          styles.alignSelf,
          styles.MenuProfilStyle,
          styles.fontSizeTitle,
          styles.marginBottom,
        ]}
      >
        {u_data?.u_data?.fullName}
      </Text>
      <Text
        style={[
          styles.bold,
      ...
      >
        (+225) {u_data?.u_data?.number}
      </Text>
      <TouchableOpacity
        onPress={() =>
          navigateAndSimpleReset('Index', {
            u_dataV: JSON.stringify(u_data),
      ...
      {u_data?.u_data?.accountType == Fr.C && (
        <TouchableOpacity
          ...
          style={styles.containItemNavigation}
        >
          <Icon size={30} color="black" name="view-dashboard-outline" />
          <Text style={[styles.MenuProfilStyle, styles.textEquilibryMargin]}>
            Tableau de bord
          </Text>
        </TouchableOpacity>
      ...
      {u_data?.u_data?.accountType == 5 && (
        <TouchableOpacity
          onPress={() =>
            navigateAndSimpleReset('PaymentMethode', {
              u_dataV: u_data,
              timestamp: new Date().getTime(),
            })
          }
          style={styles.containItemNavigation}
        >
          <Icon size={30} color="black" name="cash" />
          <Text style={[styles.MenuProfilStyle, styles.textEquilibryMargin]}>
            Mode de paiement
          </Text>
        </TouchableOpacity>
      ...
      {u_data?.u_data?.accountType == Fr.C && (
        <TouchableOpacity
          onPress={() =>
            props.navigation.navigate('Securite', {
              u_dataV: u_data,
              timestamp: new Date().getTime(),
            })
          }
          style={styles.containItemNavigation}
        >
          <Icon size={30} color="black" name="security" />
          <Text style={[styles.MenuProfilStyle, styles.textEquilibryMargin]}>
            Securité
          </Text>
        </TouchableOpacity>
