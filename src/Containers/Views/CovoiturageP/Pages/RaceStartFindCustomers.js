...
const RaceStartFindCustomers: () => Node = ({ navigation, route }) => {
  ...
  React.useEffect(async () => {
    ...
    let bodyContent = JSON.stringify({
      available: 'Non',
      uidDrivers: await u_data?.id,
      uidLat: await curentPosition.latitude,
      numberDrivers: u_data?.number,
      uidLng: curentPosition?.longitude,
      id: CustomersAvailableData?.id,
      state: 'in progress',
    })

    fetch('https://api.prumad.com/_race/Race_covoiturage/OrdersAccepte', {
      method: 'POST',
      body: bodyContent,
      headers: headersList,
    })
      ...
      .then(function (data) {
        console.log(data)
        setdetailsCurrentOrders(data.data)
      })
  ...
  React.useEffect(() => {
    const interval = setInterval(() => {
      ...
      fetch(
        'https://prumad.com/API/index2.php?OrdersUpdateDriversLocations',
        requestOptions,
      )
      ...
      <View style={styles.container}>
        ...
        >
          <MapViewDirections
            ...
            destination={{
              latitude: Number(detailsCurrentOrders?.whereYouAreLat),
              longitude: Number(detailsCurrentOrders?.whereYouAreLng),
            }}
        ...
        >
          ...
          <View>
            ...
            >
              {detailsCurrentOrders?.whereYouAre.split(' ')[0]}
            </Text>
            <Text
              style={{
                fontSize: 15,
          ...
          >
            ...
            >
              ...
              <View>
                ...
                >
                  Prendre{' '}
                  {detailsCurrentOrders?.userData?.fullName.split(' ')[0]}
                </Text>
              </View>
              <TouchableOpacity
                onPress={() =>
                  Linking.openURL(
                    `tel:${detailsCurrentOrders?.numberCustomers}`,
                  )
                }
                style={[
                  Generalstyle.shadow,
              ...
              >
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    fetch(
                      `https://prumad.com/API/index2.php?CancelRide=${detailsCurrentOrders?.uidCustomers}`,
                      requestOptions,
                    )
                      .then(response => response.json())
                      .then(result => {
            ...
            <TouchableOpacity
              onPress={() => {
                ...
                fetch(
                  'https://prumad.com/API/index2.php?UpdateCourseStart',
                  requestOptions,
                )
                  .then(response => response.json())
                  .then(result => {
