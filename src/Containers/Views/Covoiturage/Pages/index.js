...
const AccueilCovoiturage: () => Node = ({ navigation, route }) => {
  ...
  const [TimeOutOrders, setTimeOutOrders] = React.useState(130)
...
*/
  ...
  if (LatLongLocationSelected?.lat && LatLongLocationSelected2?.lat) {
    const KilometersCheck = geolib.getDistance(
      {
        latitude: LatLongLocationSelected?.lat,
        longitude: LatLongLocationSelected?.lng,
      },
      {
        latitude: LatLongLocationSelected2?.lat,
        longitude: LatLongLocationSelected2?.lng,
      },
    )
    let PriceDisplay
    const ConvertKM = geolib.convertDistance(KilometersCheck, 'km')
    ConvertKM > 1
      ? (PriceDisplay = ConvertKM * 300)
      : (PriceDisplay = ConvertKM * 0.3)
  ...
  function CreateOrders() {
    if (LatLongLocationSelected?.lat && LatLongLocationSelected2?.lat) {
      ...
      kT > 1 ? (price = kT * 300) : (price = kT * 0.3)
      ...
      let bodyContent = JSON.stringify({
        uidCustomers: u_data?.id,
        channel: 'covreservation',
        whereYouAre: whereAreYou?.description,
        Km: KTSPLIT,
        whereYouGoing: whereYouGo?.description,
        price: PRICESPLIT,
        whereYouAreLat: LatLongLocationSelected2?.lat,
        numberCustomers: u_data?.number,
        date: '',
        whereYouAreLng: LatLongLocationSelected?.lng,
        whereYouGoingLat: LatLongLocationSelected2?.lat,
        whereYouGoingLng: LatLongLocationSelected2?.lng,
      })

      fetch('https://api.prumad.com/_race/Race_covoiturage/', {
        method: 'POST',
        body: bodyContent,
        headers: headersList,
      })
        ...
        .then(result => {
          ...
          console.log(' ====> ', result)
          setlastID(result?.data?.insertId)
        })
        .catch(error => {
          console.log('error', error)
          Alert.alert(
            'Important !!!',
            "Veuillez verifier votre internet ou fermer l'application et re ouvrer là",
          )
        })
    } else {
      Alert.alert(
        'Attention !!!',
        "Vous devez remplir correctement les champs s'il vous plait !!!",
      )
    }
  }
  ...
  React.useEffect(() => {
    ...
    channel.bind('covreservation', function (data) {
      if (data?.PassagerID == u_data?.id && data?.status == 202) {
        ...
        fetch(
          'https://api.prumad.com/_race/Race_covoiturage/OrdersAccepte/' +
            data?.idRace,
          {
            method: 'GET',
            headers: headersList,
          ...
          .catch(error => console.log('error driver //', error))
      }
    })
  }, [])
  //End
  ...
  function Annuler() {
    ...
    fetch(
      `https://prumad.com/API/index2.php?CancelRide=${u_data?.id}`,
      requestOptions,
    )
      .then(response => response.json())
      .then(result => {
  ...
  function getLocationAddress(text) {
    ...
    fetch(
      `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${text}&language=fr&key=${GOOGLE_MAPS_APIKEY}`,
      requestOptions,
    )
      .then(response => response.json())
      .then(result => {
  ...
  function getLatLngInput1(text) {
    ...
    fetch(
      `https://maps.googleapis.com/maps/api/place/details/json?place_id=${text}&key=${GOOGLE_MAPS_APIKEY}`,
      requestOptions,
    )
      .then(response => response.json())
      .then(result => {
                ...
                locationPosition?.map((item, index) => (
                  ...
                  >
                    <SimpleLineIcons
                      name="location-pin"
                      size={15}
                      style={[Colors.GreenLignt, { zIndex: 6 }]}
                    />
                    <Text
                      style={{
                        fontWeight: 'bold',
            ...
            <View style={{ flex: 1 }}>
              ...
              >
                <Icon
                  name="navigation"
                  size={30}
                  style={[Colors.White, { transform: [{ rotate: '20deg' }] }]}
                />
              </TouchableOpacity>
              <View
                style={[
              ...
              >
                <View style={[{ height: windowHeight / 12 }]}>
                  <Text style={[Colors.GreenLignt, Generalstyle.bold]}>
                    Ou êtes-vous ?
                  </Text>

                  <TextInput
                    value={
                      !whereAreYou?.description
                        ? whereAreYou
                        : whereAreYou?.description
                    }
                    onFocus={e => {
                ...
                <View style={[{ height: windowHeight / 12 }]}>
                  ...
                  <TextInput
                    ...
                    onChangeText={val => setwhereYouGo(val)}
                    style={{ height: 40, color: 'black' }}
                    placeholder={'......'}
                  />
                  <View style={{ borderWidth: 0.5, opacity: 0.2 }} />
                ...
                >
                  ...
                  <Icons
                    ...
                    style={[
                      Colors.Dark,
                      { position: 'absolute', top: 3, right: 10, opacity: 0.5 },
                    ]}
                  />
                </View>
                <View style={[{ height: windowHeight / 15 }]}>
                  ...
                  >
                    <Text style={[{ color: 'white' }, Generalstyle.bold]}>
                      COMMANDER
                    </Text>
                  </TouchableOpacity>
                </View>
          ...
          {!isCommand && (
            <View style={{ flex: 1, backgroundColor: 'rgba(0, 0, 0, 0.3)' }}>
              <View>
                ...
                />
                <Image
                  source={require('../sys_pulseFecthing.gif')}
                  style={{
                    resizeMode: 'contain',
                    width: 1000,
                    alignSelf: 'center',
                ...
                >
                  ...
                  <View
                    style={{ borderWidth: 0.5, opacity: 0.2, marginTop: 20 }}
                  />
                  <View>
                    <TouchableOpacity
                      onPress={() => Annuler()}
