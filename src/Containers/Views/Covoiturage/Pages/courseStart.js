...
const CourseStart: () => Node = ({ navigation, route }) => {
  ...
  React.useEffect(() => {
    ...
    fetch(
      `https://prumad.com/API/index2.php?currentLatLongDrivers=${courseDetails?.id}`,
      requestOptions,
    )
      .then(response => response.json())
      .then(response => {
        ...
        setDet({
          latitude: parseFloat(response?.uidLat),
          longitude: parseFloat(response?.uidLng),
        })
      })
  ...
  channel.bind('GoRideEndNow', function (data) {
    if (data?.idRace == courseDetails?.id) {
      navigation.navigate('AccueilCovoiturage', {
        u_data: u_data,
        timestamp: timestamp,
      })
      setTimeout(() => {}, 2000)
  ...
  return (
    <View style={styles.container}>
      <MapView
        ...
        initialRegion={curentPositionNext}
        style={styles.map}
      >
        <MapView.Marker
          coordinate={{
            longitude: Number(courseDetails?.whereYouGoingLng),
            latitude: Number(courseDetails?.whereYouGoingLat),
          }}
        >
          <Image
            style={{
        ...
        <MapViewDirections
          ...
          destination={{
            longitude: courseDetails?.whereYouGoingLng,
            latitude: courseDetails?.whereYouGoingLat,
          }}
          ...
          >
            <View style={{ justifyContent: 'center', alignItems: 'center' }}>
              ...
              <Text style={{ color: 'black' }}>
                {courseDetails?.driverData?.carBrand}
              </Text>
              <Text />
            </View>
            <View>
              ...
              >
                <Text style={{ color: 'black' }}>
                  {courseDetails?.driverData?.numberMatricles}
                </Text>
              </View>
              <Image
                style={[
            ...
            <View>
              ...
              >
                <Image
                  ...
                  source={{
                    uri: `data:image/jpeg;base64,${courseDetails?.driverData?.photoProfil}`,
                  }}
                />
              </TouchableOpacity>
              <Text
                ...
                {!isDriverLicence ? (
                  ...
                  >
                    ...
                    <View>
                      <TouchableOpacity
                        onPress={() => {
                          Linking.openURL(
                            `tel:${courseDetails?.driverData?.number}`,
                          )
                        }}
                        style={[
                          Generalstyle.shadow,
                  ...
                  >
                    <Image
                      ...
                      source={{
                        uri: `data:image/jpeg;base64,${courseDetails?.driverData?.photoDrivingLicense}`,
                      }}
                    />
                  </TouchableOpacity>
                )}
      ...
      {IsWarn && (
        ...
        >
          ...
          >
            ...
            <TouchableOpacity
              onPress={() => Linking.openURL('tel:201')}
              style={[
                Generalstyle.shadow,
                {
                  width: '45%',
