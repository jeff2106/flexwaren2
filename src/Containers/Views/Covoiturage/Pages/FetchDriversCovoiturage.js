...
const FetchDriversCovoiturage: () => Node = ({ navigation, route }) => {
  ...
  const [Det, setDet] = React.useState({
    latitude: parseFloat(courseDetails?.whereYouAreLat),
    longitude: parseFloat(courseDetails?.whereYouAreLng),
  })
    ...
    const interval = setInterval(() => {
      ...
      fetch(
        ...
        .then(response => {
          ...
          setDet({
            latitude: Number(response?.uidLat),
            longitude: Number(response?.uidLng),
          })
        })
