...
import NotifService from '../../../../Config/NotifServices'
...
const CoursesT: () => Node = ({ navigation, route }) => {
  ...
  const sendLocalNotificationWithSound = () => {
    PushNotificationIOS.addNotificationRequest({
      ...
      sound: 'file:///System/Library/Audio/UISounds/key_press_delete.caf',
      badge: 1,
    })
  }
  ...
  channel.bind('NewBooking', function (data) {
    if (data?.iDDrivers == u_data?.id) {
      sendLocalNotification('Nouvelle reservations')
    }
  })
  /* ======= END ====== */
  ...
  function getMyAbonnment() {
    ...
    fetch(
      `https://prumad.com/API/index2.php?MonAbonnementTravailleurs=${u_data?.id}`,
      requestOptions2,
    )
      .then(response2 => response2.json())
      .then(result2 => {
        setMyAbonnment(result2)
        //console.log(result2)
      })
      .catch(error2 => console.log('error2', error2))
  }
  console.log(MyAbonnment?.dateEnd > todays)
  React.useEffect(() => {
    setIsLoading(true)
    setDrivers([])
    getAlert()
  ...
  return (
    <View style={[{ height: windowHeight, flex: 1 }]}>
      ...
      >
        <Text
          onPress={() => NotifServices.createDefaultChannels()}
      ...
      <ScrollView style={{ flex: 1 }}>
        {isLoading == false && Drivers?.length < 1 && (
          ...
          >
            ...
            {MyAbonnment?.status == 404 ? (
              <TouchableOpacity
                onPress={() =>
                  navigation.navigate('IndexPT', {
                    u_data: u_data,
                    AbonnementPrice: MyAbonnment?.price,
                    timestamp: timestamp,
                  })
                }
                style={[
        ...
        {Drivers?.map((i, k) => (
          <TouchableOpacity
            onPress={() => {
              navigation.navigate('CoursesDetailsT', {
                DDetails: i,
                u_data: u_data,
        ...
        >
          ...
          {MyAbonnment?.status != 404 && (
            <TouchableOpacity
              onPress={() =>
                navigation.navigate(
                  MyAbonnment?.dateEnd > todays ? 'NAlertT' : 'IndexPT',
                  {
                    u_data: u_data,
                    AbonnementPrice: MyAbonnment?.price,
                    timestamp: new Date().getTime(),
