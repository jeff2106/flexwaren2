...
const Courses: () => Node = ({navigation, route}) => {
  ...
  function getAlert(){
      ...
      fetch(`https://prumad.com/API/?AlertAllCourseDrivers=${Fr.IntV.toLowerCase()}&idDrivers=${u_data?.id}`, requestOptions)
  ...
  function getMyAbonnment(){
    var requestOptions2 = {
      ...
      fetch(`https://prumad.com/API/index2.php?MonAbonnementIntervilles=${u_data?.id}`, requestOptions2)
        ...
        <View
          ...
          <View style={{justifyContent:'center',alignItems:'center',marginTop:40}}>
            ...
            }]} />
            {
                ...
                <TouchableOpacity onPress={() => navigation.navigate('IndexP',{u_data : u_data,timestamp:timestamp})} style={[Colors.GreenLignt_BG,{width:'50%',justifyContent:'center',alignItems:'center',height:50,borderRadius:20}]}>
                      <Text style={[Generalstyle.bold,Colors.White]}>Créer une alerte</Text>
              </TouchableOpacity> : 
              <TouchableOpacity onPress={() => navigation.navigate('NAlert',{u_data : u_data,timestamp:timestamp})} style={[Colors.GreenLignt_BG,{width:'50%',justifyContent:'center',alignItems:'center',height:50,borderRadius:20}]}>
                      <Text style={[Generalstyle.bold,Colors.White]}>Créer une alerte</Text>
              </TouchableOpacity>
               }
          ...
          {Drivers?.map((i,k) => <TouchableOpacity onPress={() => {navigation.navigate('CoursesDetails',{DDetails: i,u_data: u_data,timestamp:new Date().getTime()})}} key={k} style={[]}>
          <CardDrivers2  item={i} />
          </TouchableOpacity>)}
        ...
        <View style={{height:20,borderBottom:1}}/>
          {
            ...
            <View style={[{
              ...
              {
                MyAbonnment?.status == "404"  && 
                <TouchableOpacity onPress={() => navigation.navigate('IndexP',{u_data : u_data,timestamp: new Date().getTime()})} style={[Colors.GreenLignt_BG,{width:'50%',justifyContent:'center',alignItems:'center',height:50,borderRadius:20}]}>
                        <Text style={[Generalstyle.bold,Colors.White]}>Créer une alerte</Text>
                </TouchableOpacity>
              }
              ...
              {
                MyAbonnment?.status != 404 && 
                <TouchableOpacity onPress={() => navigation.navigate('NAlert',{u_data : u_data,timestamp: new Date().getTime()})} style={[Colors.GreenLignt_BG,{width:'50%',justifyContent:'center',alignItems:'center',height:50,borderRadius:20}]}>
                        <Text style={[Generalstyle.bold,Colors.White]}>Créer une alerte</Text>
                </TouchableOpacity>
              }
          </View>
